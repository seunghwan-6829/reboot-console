...
### 2-1. 콘솔 v4.8 에 들어간 기능 (2026-09-26)

| 기능 | 어디서 | 파일 |
|---|---|---|
| 제품 컷 A/B 승인 | 브리프 → 사진 카드 → 제품 컷 만들기 → 나란히/겹쳐 비교 → 승인 | `product/cut_N.png`, `product/cuts.json`, `product/approved.json` (승인 컷은 본 제작에서 그대로 합성) |
| 타일 버전 되돌리기 | 검수 오른쪽 **버전** 줄 → 비교 → 되돌리기 | 바꾸기 전 자동 백업 `tiles/_history/NN/` (구 `v1`·`v_prev`·`edits` 폴더도 번호로 짝지어 표시) |
| 글자 수정 | 검수 **글자 수정(T)** → 바꿀 문구를 끌면 원래 글자 자동 인식 → 새 글자 입력 | `review.json` regions 에 `kind:"text", from, to` → AI 수정이 글자만 교체 |
| 오타 자동 검사 | 검수 **오타 검사** (Windows OCR 로 위치 + AI 가 이미지를 직접 읽어 판정) | `review/ocr/NN.json`, `review/typo.json` |
| 가독성 검사 | 타일/검수 **가독성** — 390px 폭 글자 크기 · WCAG 대비 | 기준은 설정에서 (기본 12px · 4.5:1) |
| 채널별 내보내기 | 내보내기 → 채널별 이미지 (스마트스토어 860·쿠팡 780·11번가 800·G마켓 860·자사몰 1000·원본, 장별/이어붙여 자르기) | `export/<채널>/*.jpg` |
| PSD | 내보내기 → PSD (원본 타일 + 숨긴 편집용 텍스트 레이어) | `export/psd/*.psd` |
| 이 장만 다시 / 뒤에 1장 추가 | 검수 **이 장** · 타일 그리드 카드 위 버튼 | 새 장은 `NNa.png` 로 만들고 순서(`_order`)에 끼움 |
| 사용량 계기판 | 사이드바 아래 (Max 5시간·주간 %, Higgsfield 크레딧, 대기열) → 클릭하면 이번 달 기록 | `runs.json`(프로젝트별 실행 기록) |
| 실패 자동 이어하기 | 끊기면 같은 세션(`claude --resume`)으로 최대 2번 자동, 실패 시 **이어서 하기** 버튼 | 설정 → AI 작업에서 끄기 가능 |
| 대기열 · 야간 예약 | 제작/검수 반영/한 장 대화상자의 **대기열·예약** → 지금 순서대로 / 오늘 밤 2시 / 직접 | `%APPDATA%\re-boot 콘솔\queue.json` (창을 닫으면 트레이에서 대기) |
| 클라이언트 피드백 수신함 | **피드백 붙여넣기** → AI 가 장별로 나눔(글자 교체는 원래 글자·새 글자까지) → 검수에 넣기 / 바로 AI 수정 | `feedback.json` |
| 경쟁사 분석 | 브리프 → 페이지 구성 카드 아래 URL(숨은 창으로 캡처) 또는 캡처 업로드 → 분석 → 섹션·아이디어 반영 | `ref/`, `ref.json` (지시서에 포함, 문구 복제 금지) |
| 모바일 프레임 | 타일 → **모바일**(360/390/430) | — |
| 칸반 보드 | 프로젝트 → **보드** (끌어서 단계 이동) · 정보(클라이언트·마감 D-day·메모) | `project.json` |
| 작업 기록 | 홈·프로젝트 → 작업 기록 (내보낸 파일은 열기 버튼) | `%APPDATA%\re-boot 콘솔\activity.jsonl` |
| 튜토리얼 · 단축키 | 첫 실행 3단계 안내, 어디서나 `?` | — |
